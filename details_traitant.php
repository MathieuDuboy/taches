    ...
    <div class="row">
            ...
            while($row = mysqli_fetch_array($result, true))  {
             ...
             ?> <div class="enveloppe"><?php
                 foreach($tab as $key => $petite_tache){
                       ...
                       ?>
                     <div data-idaffectation="<?php echo $petite_tache["id"]; ?>" class="petite_tache card  <?php if($petite_tache["statut"] == 'fait') echo 'bg-primary text-white';  else echo 'bg-light text-black'?>  mb-3" style="max-width: 80%;">
                       <div class="card-header">
                        <a href="details.php?id_projet=<?php echo $petite_tache["id_projet"]; ?>"><?php echo $petite_tache["nom_tache"]; ?></a>
                      </div>
      ...
      <div class="col-md-6 margin-bottom-30">
        ...
        <div class="portlet">
          ...
          <div class="portlet-body">
            <?php
            $sql = "SELECT affectation.id,affectation.id_projet,affectation.id_tache,affectation.id_sous_tache,affectation.phase,affectation.date_max,affectation.traitant,affectation.ordre,affectation.statut, taches.nom as nom_tache , sous_taches.nom as nom_sous_tache, users.first_name, users.last_name from affectation INNER JOIN taches ON affectation.id_tache = taches.id LEFT JOIN sous_taches ON affectation.id_sous_tache = sous_taches.id INNER JOIN users ON affectation.traitant = users.user_id WHERE  affectation.traitant = " . $user_id . " AND affectation.statut = 'fait' ORDER by affectation.date_max";
            $result=mysqli_query($db,$sql);
